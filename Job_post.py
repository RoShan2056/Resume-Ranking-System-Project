...
from send_email_top5 import send_email_top5
...
def ADD_JOB():
    ...
    if Last_date <= today:
        error_message = "Invalid date for deadline of the job."
...
def check_object_id_in_array(job_id, array):
    for item in array:
        if item == job_id:
            return True
    return False
...
def APPLY_JOB():
    ...
    IRS_USERS = mongo.db.IRS_USERS
    user = IRS_USERS.find_one({'_id': ObjectId(session['user_id'])})
    applied_jobs = user["applied_jobs"]
    print(applied_jobs)
    is_applied = check_object_id_in_array(job_id, applied_jobs)
    print(is_applied)
    if is_applied:
        return jsonify({"StatusCode":400,"Message":"Already applied!"})
...
def view_applied_candidates():
    global current_job_id  # accessing global variable
    job_id = request.form['job_id']
    ...
    if result_data == None:
        ...
        for i in result_data:
            ...
            cnt+=1
            user_id = i['user_id']
            user_data = IRS_USERS.find_one({"_id": ObjectId(user_id)}, {"Email": 1})
            if user_data is not None:
                Email_list.append(user_data['Email'])
        result[0]=cnt
        print("Email_list",Email_list)
        existing_doc = Top_Apply.find_one({"job_id": job_id})
        
        if existing_doc:
            print("Found Existing")
            Top_Apply.delete_one({"_id": existing_doc["_id"]})
            print("Existing Deleted")
        Top_Apply.insert_one({"job_id": job_id, "Email_list": Email_list})
        print("Top Email Added")
        print("Result",result,type(result))
        return result

@job_post.route("/send_email_top5",methods=["POST","GET"])
def send_email_top5_route():
    global current_job_id
    result_data = Top_Apply.find({"job_id":current_job_id},{"Email_list":1})
    job = JOBS.find_one({"_id": ObjectId(current_job_id)})
    job_title = job['Job_Profile']
    job_company = job['CompanyName']

    email_list = []

    for doc in result_data:
        email_list += doc["Email_list"]

    print("email_list",email_list)
    email_list_top5 = email_list[:5]
    print("email_list_top5",email_list_top5)
    send_email_top5(email_list_top5, job_title,job_company)
    return "Email sent!"
